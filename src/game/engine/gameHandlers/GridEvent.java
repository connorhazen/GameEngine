...
public class GridEvent implements Event {
  ...
  public GridEvent(List<String> operation, List<String> interaction, List<String> rules,
      Action ifExecute, Action ifNoExecute) throws XMLException {
    ...
    myOperation = (Operation) ObjectMaker
        .createObjectOf(operation.get(0), OPERATION_PACKAGE, operationParams);
    myInteraction = (Interaction) ObjectMaker
        .createObjectOf(interaction.get(0), INTERACTION_PACKAGE, interactionParams);
  ...
  public Grid execute(Grid currentGrid, Action action) {
    ...
    myInteraction.setGrid(copiedGrid, action);
