...
public class Rule2048 implements Rules {
  ...
  public boolean canPerform(List<MutableCell> currCells) {
    boolean neitherMarked = !(currCells.get(0).isMarked() || currCells.get(1).isMarked());
    boolean cellsEqualButNotZero = (currCells.get(0).getValue() == currCells.get(1).getValue())
        && currCells.get(0).getValue() != 0;
